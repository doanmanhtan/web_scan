...
const ReportsPage = () => {
  ...
  const navigate = useNavigate();
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5); // Đổi từ 10 xuống 5
  const [searchTerm, setSearchTerm] = useState('');
  const [anchorEl, setAnchorEl] = useState(null);
  const [selectedReportId, setSelectedReportId] = useState(null);
  const [reports, setReports] = useState([]);
  const [totalReports, setTotalReports] = useState(0); // Thêm state cho tổng số thực tế
  const [selectedReportDetails, setSelectedReportDetails] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  ...
  useEffect(() => {
    if (page > 0 && filteredReports.length <= page * rowsPerPage) {
      setPage(0);
    }
  }, [filteredReports.length, page, rowsPerPage]);
    ...
    const fetchReports = async () => {
      try {
        ...
        if (id) {
          ...
          // Lấy ALL reports từ backend (không limit)
          const res = await fetch('/api/scans?limit=1000', { // Tăng limit hoặc loại bỏ limit
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
          });
          ...
          const allScans = apiData.data.scans || [];
          setTotalReports(allScans.length); // Lưu tổng số thực tế
          
          const mappedReports = allScans.map(scan => ({
            id: scan._id,
            name: scan.name,
            date: (() => {
              const date = new Date(scan.createdAt);
